...
// todo: シグナルの捕捉を追加する
int serverSocketByHostname(const char *hostnm, const char *portnm) {
  ...
  std::cerr << "addr=" << nbuf << "port=" << sbuf << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  // 引数にポート番号が指定されているか
  if (argc <= 2) {
    std::cerr << "server1 address port" << std::endl;
    return EX_USAGE;
  }
  // サーバーソケットの準備
  soc = serverSocketByHostname(argv[1], argv[2]);
  if (soc == -1) {
    std::cerr << "serverSocketByHostname(" << argv[1] << "," << argv[2]
              << "):error" << std::endl;
