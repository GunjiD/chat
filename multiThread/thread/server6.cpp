...
void acceptLoop(int soc) {
  ...
  while (1) {
    ...
    if (acc == -1) {
      ...
      // スレッド生成
      if (pthread_create(&thread_id, nullptr, sendRecvThread, (void *)acc) !=
          0) {
        std::cerr << "pthread_create" << std::endl;
      } else {
        std::cerr << "pthread_create:create:thread_id=" << thread_id
                  << std::endl;
      }
    }
  }
...
void *sendRecvThread(void *arg) {
  ...
  // 引数の取得
  acc = reinterpret_cast<intptr_t>(arg);
